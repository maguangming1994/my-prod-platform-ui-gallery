...
| 03 经营工作台 | 经营身份总览、OPC 工作台、代理星卡工作台、销售工作台 |
| 04 代理核心操作 | 星卡采购、下级代理审核、本区域销售提交与审核；代理无商品发布权限 |
...
- 代理：只经营付费星卡库存，突出星卡进货、库存批次、下级销售与经营明细；不得展示或操作普通实物商品库存。
- 销售：突出推广、客户订单、所属代理与收益。
- 综合经营账号：通过身份总览切换角色，角色工作台内部不重复放多身份导航。

### 代理库存业务边界

- “代理库存”在全系统中唯一指付费星卡库存，库存单位统一为“张”。
- 代理进货、首批进货和补货均指使用人民币采购付费星卡库存，并形成数量、单位成本、入库时间和先进先出批次。
- 耳机、面霜、箱包等普通实物商品属于 OPC 商品与实物 SKU 库存，不进入代理库存、代理进货或代理利润计算。
- 销售可以推广 OPC 的普通商品，但销售推广关系不使所属代理取得、持有或管理这些商品库存。
- 03、04 号画板中出现的普通实物代理采购与库存内容属于旧稿错误；开发与后续修图统一以 11、16、17 号画板及本节规则为准。

### 已确认的跨页面规则

- 同一 App 固定使用“首页、商城、发现、资产、我的”五项主导航；OPC、代理、销售、子账号和讲师通过经营身份工作台进入，不创建角色独立 App。
- 课程详情保留“立即购买”的目标设计，但购买闭环未完成时按钮必须禁用并说明原因。
- 资产中心规划由持牌支付机构承载的真实现金储值钱包；它与人民币收益账户、星卡账户分开显示、分开记账。星链只维护业务账本镜像，不直接沉淀客户充值资金；未经合规和真实渠道验收不得开放充值。
- 会员当前只展示固定期限产品，不展示连续包月、自动续费或代扣签约。
- 所有用户可见业务编号必须随机且不可推测，不得使用日期加顺序号作为样例。
- 代理审核通过只开放 72 小时首批进货资格；支付成功后由服务端原子竞争唯一席位，失败全额原路退款。
- 月度页面只表达经营结算和预计税费，法定税务报送按季度单独展示。
